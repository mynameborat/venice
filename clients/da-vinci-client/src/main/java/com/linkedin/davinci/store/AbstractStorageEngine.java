...
public abstract class AbstractStorageEngine<Partition extends AbstractStoragePartition>
  ...
  /**
   * Ingest external SST files into a partition.
   * This is used for blob-based push where SST files are generated by VPJ
   * and transferred to servers via HDFS.
   *
   * @param partitionId The partition to ingest into
   * @param sstFilePaths List of paths to SST files to ingest
   * @param isRmd Whether these are replication metadata SST files
   */
  public synchronized void ingestExternalFiles(int partitionId, java.util.List<String> sstFilePaths, boolean isRmd) {
    AbstractStoragePartition partition = partitionList.get(partitionId);
    if (partition == null) {
      throw new com.linkedin.venice.exceptions.VeniceException(
          "Partition " + partitionId + " doesn't exist, cannot ingest external files");
    }
    partition.ingestExternalFiles(sstFilePaths, isRmd);
  }
