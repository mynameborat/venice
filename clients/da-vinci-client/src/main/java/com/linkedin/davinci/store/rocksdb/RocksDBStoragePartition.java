...
public class RocksDBStoragePartition extends AbstractStoragePartition {
  ...
  /**
   * Ingest external SST files directly into RocksDB.
   * This method is used for blob-based push where SST files are generated by VPJ
   * and transferred to servers via HDFS.
   *
   * @param sstFilePaths List of paths to SST files to ingest
   * @param isRmd Whether these are replication metadata SST files
   */
  @Override
  public synchronized void ingestExternalFiles(List<String> sstFilePaths, boolean isRmd) {
    makeSureRocksDBIsStillOpen();
    if (readOnly) {
      throw new VeniceException(
          "Cannot ingest external files while database is opened in read-only mode for replica: " + replicaId);
    }
    if (sstFilePaths == null || sstFilePaths.isEmpty()) {
      LOGGER.info("No SST files to ingest for store: {}, partition: {}", storeNameAndVersion, partitionId);
      return;
    }

    LOGGER.info(
        "Starting {} SST file ingestion for store: {}, partition: {}, files: {}",
        isRmd ? "RMD" : "data",
        storeNameAndVersion,
        partitionId,
        sstFilePaths);
    ...
    try (IngestExternalFileOptions ingestOptions = new IngestExternalFileOptions()) {
      // Move files to RocksDB directory to avoid duplicates
      ingestOptions.setMoveFiles(true);

      // Get appropriate column family handle
      ColumnFamilyHandle columnFamilyHandle;
      if (isRmd && columnFamilyHandleList.size() > 1) {
        // REPLICATION_METADATA_COLUMN_FAMILY_INDEX = 1
        columnFamilyHandle = columnFamilyHandleList.get(1);
      } else {
      ...
      LOGGER.info(
          "Successfully completed {} SST file ingestion for store: {}, partition: {}, files count: {}",
          isRmd ? "RMD" : "data",
          storeNameAndVersion,
          partitionId,
          sstFilePaths.size());
      ...
      throw new VeniceException(
          "Failed to ingest external SST files for replica: " + replicaId + ", isRmd: " + isRmd,
          e);
    }
  }
